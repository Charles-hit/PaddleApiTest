...
class TestMaxDevelopCase1_FP32(unittest.TestCase):
    def init_dtype(self):
        self.dtype = "float32"

    def init_params(self):
        self.x_shape = [10, 2, 256]
        self.axis = None
        self.keepdim = False
        self.dout_shape = []
        self.init_dtype()
    ...
    def setUp(self):
        ...
        out_torch, out_grads_torch = self.cal_torch_res(
            x_torch, self.axis, self.keepdim, dout_torch
        )
        del x_torch
        del dout_torch
    ...
    def cal_torch_res(self, x, axis, keepdim, dout):
        x_t = x
        dout_t = dout
        if self.dtype == "bfloat16":
        ...
        if axis is None and keepdim is False:
            ...
            out = torch.max(x_t, dim=axis, keepdim=keepdim)
        out_grads = torch.autograd.grad([out], [x], grad_outputs=[dout_t])
        if self.dtype == "bfloat16":
            out = out.to(dtype=torch.float32)
        return out, out_grads

    def cal_eager_res(self, x, axis, keepdim, dout):
        x_t = x
        dout_t = dout
        if self.dtype == "bfloat16":
            x_t = paddle.cast(x, dtype="uint16")
            dout_t = paddle.cast(dout, dtype="uint16")
        out = paddle.max(x_t, axis=axis, keepdim=keepdim)
    ...
    def test_eager_accuracy(self):
        ...
        out_eager, out_grads_eager = self.cal_eager_res(
            x_eager, self.axis, self.keepdim, dout_eager
        )
        del x_eager
        del dout_eager
...
class TestMaxDevelopCase1_FP16(TestMaxDevelopCase1_FP32):
    def init_dtype(self):
        self.dtype = "float16"

class TestMaxDevelopCase1_BF16(TestMaxDevelopCase1_FP32):
    def init_dtype(self):
        self.dtype = "bfloat16"
