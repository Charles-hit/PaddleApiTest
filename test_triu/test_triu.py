...
class TestTriuDevelopCase1_FP32(unittest.TestCase):
    def setUp(self):
        self.init_params()
        self.init_threshold()
        ...
        out_torch, out_grads_torch = self.cal_torch_res(
            x_torch, self.diagonal, dout_torch
        )
        del x_torch
        del dout_torch
    ...
    def init_np_inputs_and_dout(self):
        # init np array
        self.np_input = np.random.random(size=self.input_shape).astype("float32") - 0.5
        self.np_dout = np.random.random(size=self.input_shape).astype("float32") - 0.5
    ...
    def cal_torch_res(self, x, diagonal, dout):
        if self.dtype == "bfloat16":
            x = x.to(dtype=torch.bfloat16)
            dout = dout.to(dtype=torch.bfloat16)
        out = torch.triu(x, diagonal=diagonal)
    ...
    def cal_eager_res(self, x, diagonal, dout):
        if self.dtype == "bfloat16":
            x = paddle.cast(x, dtype="uint16")
            dout = paddle.cast(dout, dtype="uint16")
        out = paddle.triu(x, diagonal=diagonal)
    ...
    def test_eager_accuracy(self):
        ...
        out_eager, out_grads_eager = self.cal_eager_res(
            x_eager, self.diagonal, dout_eager
        )
        del x_eager
        del dout_eager
