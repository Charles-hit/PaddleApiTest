...
class TestClipDevelopCase1_FP32(unittest.TestCase):
    def init_dtype(self):
        self.dtype = "float32"
    def init_params(self):
        self.x_shape = [1, 4, 40, 72]
        self.dout_shape = [1, 4, 40, 72]
        self.param_min = -30.0
        self.param_max = 20.0
        self.init_dtype()
    ...
    def setUp(self):
        ...
        out_torch, out_grads_torch = self.cal_torch_res(
            x_torch, self.param_min, self.param_max, dout_torch
        )
        del x_torch
        del dout_torch
    ...
    def cal_torch_res(self, x, param_min, param_max, dout):
        x_t = x
        dout_t = dout
        if self.dtype == "bfloat16":
            x_t = x.to(dtype=torch.bfloat16)
            dout_t = dout.to(dtype=torch.bfloat16)
        out = torch.clip(x_t, param_min, param_max)
        out_grads = torch.autograd.grad([out], [x], grad_outputs=[dout_t])
        if self.dtype == "bfloat16":
            out = out.to(dtype=torch.float32)
        return out, out_grads

    def cal_eager_res(self, x, param_min, param_max, dout):
        x_t = x
        dout_t = dout
        if self.dtype == "bfloat16":
            x_t = paddle.cast(x, dtype="uint16")
            dout_t = paddle.cast(dout, dtype="uint16")
        out = paddle.clip(x_t, param_min, param_max)
    ...
    def test_eager_accuracy(self):
        ...
        out_eager, out_grads_eager = self.cal_eager_res(
            x_eager, self.param_min, self.param_max, dout_eager
        )
        del x_eager
        del dout_eager
...
class TestClipDevelopCase1_FP16(TestClipDevelopCase1_FP32):
    def init_dtype(self):
        self.dtype = "float16"

class TestClipDevelopCase1_BF16(TestClipDevelopCase1_FP32):
    def init_dtype(self):
        self.dtype = "bfloat16"
