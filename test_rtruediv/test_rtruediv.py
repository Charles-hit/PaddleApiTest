...
class TestRtruedivDevelopCase1_FP32(unittest.TestCase):
    def setUp(self):
        self.init_params()
        self.init_threshold()
        self.init_np_inputs_and_dout()
        x_torch, dout_torch = self.gen_torch_inputs_and_dout()
        out_torch, out_grads_torch = self.cal_torch_res(x_torch, self.y, dout_torch)
        del x_torch
        del dout_torch
    ...
    def init_np_inputs_and_dout(self):
        # init np array
        self.np_x = np.random.random(size=[1024, 1]).astype("float32") - 0.5
        self.y = (np.random.random(size=[1]).astype("float32") - 0.5)[0]
        self.np_dout = np.random.random(size=[1024, 1]).astype("float32") - 0.5
    ...
    def test_eager_accuracy(self):
        ...
        out_eager, out_grads_eager = self.cal_eager_res(x_eager, self.y, dout_eager)
        del x_eager
        del dout_eager
